/*
 *
 *Create and export configuration variables
 *For Online Pizza Order Delivery
 *
 *Contains config variables for http(s) port(s)
 *Which environment will be running (staging or production)
 *Hashing secret
 *Maximum amount of carts allowed to be saved
 *Keys for APIs
 *
 */

//Container for all the environments to be exported
var environments = {};

//Staging (default) environment
environments.staging = {
  'httpPort' : 3000,
  'httpsPort' : 3001,
  'envName' : 'staging',
  'hashingSecret' : 'thisIsASecret',
  'maxCarts' : 5,
  'stripe' : {
      'secretKey' : '',
      'publicKey' : ''
  }
};


//Production environment
environments.production = {
  'httpPort' : 5000,
  'httpsPort' : 5001,
  'envName' : 'production',
  'hashingSecret' : 'thisIsASecret',
  'maxCarts' : 5,
  'stripe' : {
      'secretKey' : '',
      'publicKey' : ''
  }
};


//Determine which environment should be exported
var currentEnvironment = typeof(process.env.NODE_ENV) == 'string'
                                  ? process.env.NODE_ENV.toLowerCase() : '';

//Check that the current environment is one of the environments above
//If not default to staging
var environmentToExport = typeof(environments[currentEnvironment]) == 'object' 
                                  ? environments[currentEnvironment] : environments.staging;


//Export the module
module.exports = environmentToExport;
