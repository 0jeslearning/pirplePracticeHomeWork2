...
//post to /order url from there the saved cart will become an order
//Currently the only required data is listed below with no optional data
//This may change 
//Required data: product,quantity
//Optional data: none
cartHandlers._cart.post = function(data,callback) {
  ...
  //If product and quantity have been entered continue to process cart
  if(product && quantity) {
            ...
            if(!err && userData) {
              ...
              if(userSession.length < config.maxCarts) {
                ...
                var cartObject = {
                  'cartId' : sessionId,
                  'userPhone' : userPhone,
                  'product' : menuData[product].product,
                  'price' : menuData[product].price,
                  'quantity' : quantity,
                  'total' : itemsTotal,
                }
    ...
    callback(400,{'Error' : 'Missing required inputs, or inputs are invalid. Valid inputs are: Product, Quantity, Place Order (true or false)'}); 
  } 
...
cartHandlers._cart.delete = function(data,callback) {
  ...
  if(cartId) {
    ...
    callback(400,{'Error' : 'Query is missing required field or entered incorrectly'});
  }
